...
// kdo je teď na tahu - začíná kolečko, tedy já
...
const AITah = async (herniPole) => {
  ...
  // přidáme malou prodlevu pro realističtější dojem
...
// pokud někdo vyhraje, zobrazí se hláška a aktualizuje se hra
  ...
  if (vitez === 'o' || vitez === 'x') {
    ...
    const pozice = await AITah(herniPole);
    const index = pozice.x + pozice.y * 10;
    const vybranePolicko = document.querySelectorAll('.cell')[index];

    vybranePolicko.classList.add('board__field--cross');
    vybranePolicko.disabled = true;
